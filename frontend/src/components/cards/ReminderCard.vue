  ...
  >
    <v-card-actions :class="`px-0 bg-${reminder.character.color}-lighten-1`">
      <v-list-item class="w-100">
        ...
        <v-list-item-title class="character-name">{{ reminder.character.name }}</v-list-item-title>

        <v-list-item-subtitle class="reminder-date"> {{ reminder.date }} </v-list-item-subtitle>

        <template v-slot:append>
          <div class="justify-self-end">
            <v-menu >
              <template v-slot:activator="{ props }">
                <v-btn class="menu-btn" icon="mdi-dots-vertical" v-bind="props"></v-btn>
              </template>
